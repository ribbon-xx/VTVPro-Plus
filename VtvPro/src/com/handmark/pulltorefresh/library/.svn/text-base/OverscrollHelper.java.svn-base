...
final class OverscrollHelper
{
     ...
     *        should treat the other values. Needed for WebView as it doesn't
     *        always scroll back to it's edge. 0 = no fuzziness.
        ...
        if (view.isPullToRefreshOverScrollEnabled())
        {
            ...
            if (mode != Mode.DISABLED && !isTouchEvent && deltaY != 0)
            {
                ...
                else if (newY > (scrollRange + fuzzyThreshold))
                {
                    ...
                    if (mode.canPullUp())
                    {
                        view.setHeaderScroll((int) (scaleFactor * (view.getScrollY()
                            + newY - scrollRange)));
                    }
                }
